...
import { PlusCircle, Trash2, Loader2, ChevronDown, ChevronUp, ArrowUp, ArrowDown } from "lucide-react";
...
function QuestionItem({ questionIndex, totalQuestions, control, onRemove, onMoveUp, onMoveDown }: QuestionItemProps) {
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div className="flex flex-col gap-0.5 shrink-0">
            <Button type="button" size="icon" variant="ghost" className="h-5 w-5"
              onClick={onMoveUp} disabled={questionIndex === 0}>
              <ArrowUp className="h-3 w-3" />
            </Button>
            <Button type="button" size="icon" variant="ghost" className="h-5 w-5"
              onClick={onMoveDown} disabled={questionIndex === totalQuestions - 1}>
      ...
      {!collapsed && (
        <CardContent className="space-y-4">
          ...
          <FormField control={control} name={`questions.${questionIndex}.text`}
            render={({ field }: { field: ControllerRenderProps<CreatePollFormValues, `questions.${number}.text`> }) => (
              <FormItem>
          ...
          <div className="space-y-2">
            ...
            {optionFields.map((optionField, optionIndex) => (
              <FormField key={optionField.id} control={control}
                ...
                render={({ field }: { field: ControllerRenderProps<CreatePollFormValues, `questions.${number}.options.${number}.text`> }) => (
                  <FormItem>
                    <FormControl>
...
export default function CreatePollPage() {
  ...
  const { control, handleSubmit, watch, setValue, formState: { isSubmitting } } = form;
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">Build your poll, set an expiry, and share the link.</p>
      </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6" noValidate>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <FormField control={control} name="title"
                render={({ field }: { field: ControllerRenderProps<CreatePollFormValues, "title"> }) => (
                  <FormItem>
                    <FormLabel>Title</FormLabel>
                    <FormControl><Input placeholder="e.g. Team Lunch Preferences" {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
              <FormField control={control} name="description"
                render={({ field }: { field: ControllerRenderProps<CreatePollFormValues, "description"> }) => (
                  <FormItem>
                    <FormLabel>Description <span className="text-muted-foreground font-normal">(optional)</span></FormLabel>
                    <FormControl><Textarea placeholder="Add context for your respondents…" rows={3} {...field} /></FormControl>
                    <FormMessage />
                  </FormItem>
                )}
              />
              <FormField control={control} name="expiresAt"
                render={({ field }: { field: ControllerRenderProps<CreatePollFormValues, "expiresAt"> }) => (
                  <FormItem>
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">Only authenticated users can submit responses</p>
                </div>
                <div className="flex items-center gap-2">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">Respondent identity is not stored</p>
                </div>
                <div className="flex items-center gap-2">
          ...
          <div className="space-y-3">
            <h2 className="landing-heading text-base font-semibold">
              Questions <span className="text-muted-foreground font-normal text-sm">({questionFields.length})</span>
            </h2>
          ...
          <div className="flex justify-end gap-3 pt-2">
            ...
            <Button type="submit" disabled={isSubmitting}>
              {isSubmitting ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Creating…</> : "Create poll"}
            </Button>
          </div>
